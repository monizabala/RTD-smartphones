...
SIMPLES DIFERENCIAS CON LA NETWORK
@author: mjjimene
"""
...
# vector b
    ...
    for i in range(len(data_BS_coor)):
        ro_BS_Base=math.sqrt((data_BS_coor[i][0]-x_UTM_cal_Net_Base)**2+(data_BS_coor[i][1]-y_UTM_cal_Net_Base)**2+(data_BS_coor[i][2]-z_UTM_cal_Net_Base)**2)
        ro_BS_Rover=math.sqrt((data_BS_coor[i][0]-x_UTM_cal_Net_R)**2+(data_BS_coor[i][1]-y_UTM_cal_Net_R)**2+(data_BS_coor[i][2]-z_UTM_cal_Net_R)**2)
        ro_RB=ro_BS_Rover-ro_BS_Base
        
        x_UTM_BS=data_BS_coor[i][0]
        y_UTM_BS=data_BS_coor[i][1]
        z_UTM_BS=data_BS_coor[i][2]
        ...
        for p in range(len(data_geo_coord_network_Base)):
             ...
             R_BS_Base=math.sqrt((data_BS_coor[i][0]-x_UTM_B)**2+(data_BS_coor[i][1]-y_UTM_B)**2+(data_BS_coor[i][2]-data_geo_coord_network_Base[p][2])**2)
             ...
             R_BS_Rover=math.sqrt((data_BS_coor[i][0]-x_UTM_R)**2+(data_BS_coor[i][1]-y_UTM_R)**2+(data_BS_coor[i][2]-data_geo_coord_network_Rover[p][2])**2)
             R_RB=R_BS_Rover-R_BS_Base
             z_UTM_R=data_geo_coord_network_Rover[p][2]
             
             b=ro_RB-R_RB+clk_rec_err_5G
             vector_b.append([b])
        
    # matriz A
    
             dx_rm_BS = x_UTM_cal_Net_R - x_UTM_BS
             dy_rm_BS = y_UTM_cal_Net_R - y_UTM_BS
             dz_rm_BS = z_UTM_cal_Net_R - z_UTM_BS                   
    
                   
             ax_s1_m=(dx_rm_BS/(ro_BS_Rover))
             ay_s1_m=(dy_rm_BS/(ro_BS_Rover))
             az_s1_m=( dz_rm_BS/(ro_BS_Rover))
    
             A=([(ax_s1_m), (ay_s1_m), (az_s1_m),1])
             matrix_A.append(A)
    ...
    matrix_A_P=numpy.matmul(diag_weigh_network,matrix_A)
    vector_b_P=numpy.matmul(diag_weigh_network,vector_b)
            # 
    ...
    it=it+1
    print('inc_comp_x_P, inc_comp_y_P,inc_comp_z_P, it',inc_comp_x_P, inc_comp_y_P,inc_comp_z_P, it)
    x_UTM_cal_Net_R=x_comp_P
    y_UTM_cal_Net_R=y_comp_P
    z_UTM_cal_Net_R=z_comp_P
    print('inc_comp_x, inc_comp_y,inc_comp_z, it',inc_comp_x, inc_comp_y,inc_comp_z, it)
    x_UTM_cal_Base_R=x_comp
    y_UTM_cal_Base_R=y_comp
    z_UTM_cal_Base_R=z_comp
    
    # clk_rec_err_5G=clk_rec_err_5G+dr_SD_P[3]
...
print('inc_rot_x_net, inc_rot_y_net,inc_final_z_net',inc_rot_x, inc_rot_y,inc_comp_z_P)
